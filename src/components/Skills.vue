<template>
  <section-layout title="skills">
    <v-layout row wrap>
      <v-flex xs12 md6 v-for="item in barDetails" :key="item.name">
        <div class="mb-4 mx-5">
          <v-layout row wrap>
            <i
            ...
            />
            <div class="subheading blue-grey--text text--darken-3 ml-2">
              {{ item.name }}
            </div>
          </v-layout>
          <v-progress-linear
...
export default {
  ...
  data: () => ({
    items: [
      ...
      { name: 'python', icon: 'devicon-python-plain', value: 91 },
